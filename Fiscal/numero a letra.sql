...
(  
  @Numero NUMERIC(19,4),  
  @NumeroLetra VARCHAR(100) OUTPUT  
 )  
AS  
BEGIN  
  SET NOCOUNT ON  
  --DECLARE @Numero NUMERIC(19,4)=14.21  
  --DECLARE @NumeroLetra VARCHAR(100)  
 ...
 --print @lcCadena  
  ...
  SET @NumeroLetra=(   
  SELECT RTRIM(@lcRetorno) + CONVERT(VARCHAR,  
  CASE   
 WHEN LEN(LTRIM(STR(@lnFraccion,2))) = 1 THEN '0'+LTRIM(STR(@lnFraccion,2))  
